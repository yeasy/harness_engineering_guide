...
```mermaid
graph LR
    A["pending<br/>待执行"] -->|调度| B["running<br/>执行中"]
    B -->|成功| C["completed<br/>成功完成"]
    B -->|错误| D["failed<br/>执行失败"]
    B -->|取消| E["killed<br/>被杀死"]
    D -.->|重试| B
    E -.->|重新入队| A
```
...
graph TD
    ...
    B["Task 1<br/>文本预处理<br/>local_bash"]
    B1["获取论文PDF"]
    B2["提取纯文本和元数据"]

    C["Task 2a<br/>内容分析<br/>local_agent"]
    C1["计算论文深度"]
    C2["评估新颖性"]
    C3["分析逻辑完整性"]

    D["Task 2b<br/>引用分析<br/>local_agent"]
    D1["解析引用列表"]
    D2["统计引用数量和质量"]

    E["Task 2c<br/>格式审查<br/>local_bash"]
    E1["检查排版规范"]
    E2["验证图表清晰度"]

    F["Task 3<br/>综合评估<br/>local_agent"]
    F1["整合分析结果"]
    F2["生成最终报告"]
