...
class PathValidator:
    ...
    def _decode_all_encodings(self, path: str) -> str:
        ...
        while previous != current:
            ...
            # 安全检查：最多解码20次（防止无限循环）
            if current.count('%') > 0 and current.count('%') == previous.count('%'):
                break
...
路径校验涉及多次字符串操作和可能的文件系统调用（realpath），需要考虑性能：
```python
import time
...
class CachedPathValidator(PathValidator):
    ...
    def __init__(self, base_path: str = "/tmp/agent", cache_size: int = 1000):
...
**本节总结**：路径校验是防护路径穿越的关键。通过5层递进式防护（长度、解码、Unicode、平台、realpath），可以有效应对多种已知和未知的编码攻击。关键是：**规范化后必须使用realpath，并检查最终路径是否在边界内**。
