...
```mermaid
flowchart TD
    A["Manual Only<br/>完全人工"] -->|每一步都需要批准| B["Approve Always<br/>每步审批"]
    B -->|每个操作都需要批准| C["Approve Once<br/>一次审批"]
    C -->|任务开始时批准一次| D["Ask First<br/>事前询问"]
    D -->|首次询问，后续自动| E["Auto with Notification<br/>自动+通知"]
    E -->|自动执行并发送通知| F["Full Trust<br/>完全信任"]
...
**本节总结** ：权限与沙箱是防护的两个维度。权限控制 **是否允许** 行，沙箱限制 **执行的破坏范围** 。两者结合形成纵深防护。
