...
Claude Code 的公开权限模式包括 `default`、`acceptEdits`、`plan`、`auto`、`dontAsk` 和 `bypassPermissions`。其中 `default` 主要自动允许读取并在编辑或执行时提示，`acceptEdits` 可自动批准工作区内文件编辑和常见文件系统命令，`plan` 用于只读研究和计划，`auto` 在后台安全检查下减少提示，`dontAsk` 只运行预批准工具，`bypassPermissions` 跳过权限层，官方建议仅在隔离环境中使用。以下将其扩展为更细粒度的六级模型，便于理解权限决策的设计空间：
...
Claude Code 的 `auto` 模式使用独立的后台分类器检查待执行动作；读操作和工作区内文件编辑通常跳过分类器，其他动作会进入安全检查。以下代码是通用风险评估伪代码，用于说明 Harness 可怎样组织权限决策，不代表 Claude Code 的真实内部 SDK 或实现：
...
tool_func, args, kwargs = pickle.loads(sys.stdin.buffer.read())

try:
    output = tool_func(*args, **kwargs)
    sys.stdout.buffer.write(pickle.dumps(output))
except Exception as exc:
    sys.stderr.buffer.write(pickle.dumps(exc))
    sys.exit(1)
"""
    ...
    stdout, stderr = proc.communicate(
        pickle.dumps((tool_func, args, kwargs)),
        timeout=30
    )
    if proc.returncode != 0:
        raise pickle.loads(stderr)
