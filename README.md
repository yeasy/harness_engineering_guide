...
2026 年之初，业界逐渐形成共识：**决定智能体成败的，往往不是底层大模型的能力，而是包裹在模型外围的 Harness 系统**。

“Harness 工程”（Harness Engineering）这一概念开始进入主流视野。多个生产级智能体项目（Agent）的经验表明：模型不再是差异化因素，围绕模型构建的工程系统才是。

笔者认为：`智能体 = 大模型 + Harness`——大模型提供推理的“大脑”，Harness 提供执行、记忆与安全保障的“身体”。验证循环、重试策略、状态管理、检查点恢复——这些 Harness 层的能力，决定了一个智能体系统能否从原型走向生产。软件工程社区开始系统化地探讨这一工程范式。
...
graph LR
    ...
    Ch1 --> Role1["平台工程师<br/>第1-3章 → 第4-7章 → 第10-11章"]
    Ch1 --> Role2["应用架构师<br/>第1-3章 → 第8-9章 → 第12章"]
    Ch1 --> Role3["全栈开发者<br/>第1-2章 → 第4-7章（含 MiniHarness）"]
    Ch1 --> Role4["技术决策者<br/>第1-3章 → 第12章 → 第14章"]
