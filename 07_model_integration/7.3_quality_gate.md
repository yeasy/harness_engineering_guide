...
### 7.4 规划-生成-评估三角色分离（PGE Pattern）

在传统的单一模型Agent中，智能体需要同时完成三项职责：分析任务、生成解决方案、评估结果。这导致一个常见问题：**自我确认偏差**——生成器很难公正地评估自己的工作。

Anthropic提出的**规划-生成-评估（Planner-Generator-Evaluator, PGE）模式**通过角色分离和潜在的不同模型配置，显著提升输出质量。
...
```mermaid
graph LR
    A["用户请求"] -->|分析意图| B["规划者<br/>生成执行计划"]
    B -->|步骤序列<br/>验证标准| C["生成者<br/>执行任务"]
    C -->|原始输出| D["评估者<br/>独立验证"]
    D -->|是否通过| E{结果判断}
    E -->|通过| F["返回结果"]
    E -->|失败| G["反馈给生成者"]
    G -->|修正方案| C
