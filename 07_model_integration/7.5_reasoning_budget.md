...
> **更新**: Extended Thinking 的手动模式（`type: "enabled"`）在 Claude Opus 4.8/4.7 上已被移除（请求返回 400 错误），在 Opus 4.6 与 Sonnet 4.6 上已弃用但仍可用；Adaptive Thinking（`type: "adaptive"`）是官方推荐方案。
...
| 模型 | 状态 |
|------|------|
| Claude Opus 5、Sonnet 5、Fable 5（当前一代） | ❌ 不支持，只提供 Adaptive Thinking |
| Claude Opus 4.8/4.7 | ❌ 不支持，返回 400 错误 |
| Claude Opus 4.6、Sonnet 4.6 | ⚠️ 已弃用但功能正常（计划迁移） |
| Claude Opus 4.5 等早期模型 | ✅ 仍支持（若无法升级） |
