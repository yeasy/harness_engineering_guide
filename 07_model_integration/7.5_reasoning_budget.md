...
> **更新**: Extended Thinking 的手动模式（`type: "enabled"`）在 Claude Fable 5.1、Mythos 5.1、Fable 5、Mythos 5、Opus 5、Sonnet 5 与 Opus 4.8/4.7 上已被移除（请求返回 400 错误），在 Opus 4.6 与 Sonnet 4.6 上已弃用但仍可用；Adaptive Thinking（`type: "adaptive"`）是官方推荐方案。反向的边界同样要注意：Sonnet 4.5、Opus 4.5、Haiku 4.5 等更早的模型只有手动模式，传 `type: "adaptive"` 会返回 400。完整对照见本节末的附注表。
...
### 附注：Extended Thinking 的模型差异

Claude 官方对手动 Extended Thinking 配置（`thinking: {type: "enabled", budget_tokens: N}`）按模型区分处理：

| 模型 | 状态 |
|------|------|
| Claude Fable 5.1、Mythos 5.1（2026-09-01 发布，当前一代）与 Fable 5、Mythos 5（已列入官方 Legacy models，仍可调用） | ❌ 不支持，返回 400 错误；思考常开，`type: "disabled"` 同样返回 400 |
| Claude Opus 5、Sonnet 5 | ❌ 不支持，返回 400 错误，只提供 Adaptive Thinking（Opus 5 在 `effort` 为 `xhigh`/`max` 时也不接受 `"disabled"`） |
