...
Claude Code 在构建消息时采用分层的Token预算管理。注意：Anthropic 官方[上下文窗口说明](https://platform.claude.com/docs/en/build-with-claude/context-windows)中既有 200K 级模型，也有 Claude Sonnet 4.6、Opus 4.6/4.7 等 1M 级模型；下面的 200K 是保守示例，不应硬编码为所有 Claude 或 Claude Code 场景的上限。
