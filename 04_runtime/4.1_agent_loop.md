...
智能体循环(智能体循环)是智能体系统的核心执行模式。经典的“思考-行动-观察”（Think-Act-Observe）循环定义如下：
...
flowchart TD
    subgraph Generator["QueryEngine.submitMessage() Generator"]
        ...
        subgraph C["Async Turn Loop （while hasWorkToDo）"]
            D["yield turn_start"] --> E["Build & Validate Context<br/>（考虑缓存边界）"]
            E --> F["Stream Model Response<br/>（yield text_delta / tool_use）"]
            F --> G["StreamingToolExecutor.execute()<br/>（并发执行工具）"]
...
flowchart TD
    subgraph SessionLoop["Session Loop（单线程）"]
        A["Intake<br/>（user msg）"] --> B["Context Assembly"]
        B --> C["Inference<br/>（Claude）"]
        C --> D["Response Streaming<br/>+ Tool Detection"]
        D --> E["Tool Execution Loop<br/>（顺序/不阻塞UI）"]
        E --> F["Persistence<br/>（会话存储）"]
        F -->|"继续下一轮（如需要）"| A
    end
```
...
两种模式的核心都是“思考-行动-观察”循环，区别在于编程模型（同步vs异步）和工具执行策略（顺序vs并发）。

理解这两种模式的设计权衡，是构建 Harness 运行时的基础。
