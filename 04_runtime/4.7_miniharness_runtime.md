...
检查点以 `session_id + call_id` 标识一次调用，显式保存 `user_id` 所代表的 principal；参数指纹也包含该 principal、工具名和参数。每次调用都先重新经过权限与护栏，再读取检查点，因此策略已改为 `DENY` 的请求无法读取旧结果。恢复时：

- `completed`：仅向同一 principal 返回已经保存的结果，不再次调用工具；
- `started`：拒绝自动重放，因为进程可能在副作用完成后、结果落盘前崩溃；
- 同一调用 ID 被其他 principal 使用，或参数指纹不同：抛出冲突错误。

JSON 存储使用同目录临时文件、`fsync` 和原子替换，并把文件权限设置为仅当前用户可读写。它解决本机进程重启后的重复调度问题，但不能让远端副作用与本地文件形成分布式事务；写操作仍应向服务端传递幂等键。
