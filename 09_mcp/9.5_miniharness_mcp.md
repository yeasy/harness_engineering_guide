...
1. 查找相同 `session_id + call_id` 的检查点；
2. 执行权限和护栏判断；
3. 在真正调用前写入 `started` 检查点；
4. 仅对配置的可恢复异常重试，并写入同一 Trace 的事件；
5. 成功或确定失败后保存可重放结果。

已完成的调用在恢复时直接复用结果，不重复副作用。若只看到 `started` 而没有完成结果，系统会拒绝盲目重放，因为远端操作可能已经成功。这一策略优先避免重复写入；它不能让“远端副作用 + 本地 JSON 检查点”获得跨系统原子性。需要严格一次性语义时，Server 仍须支持幂等键、事务或可查询的操作状态。
