...
class MiniHarnessWithMCP:
    ...
    async def initialize(self):
        ...
        await self.registry.add_server(MCPServerConfig(
            ...
            endpoint="./mcp_fs_server.py",
        ))

        # 发现工具
        tools = await self.registry.discover_tools()

    async def process_tool_call(self, tool_name: str, tool_input: str):
        # LLM输入 -> JSON解析 -> 调用 -> 返回结果
        return await self.adapter.call_tool_from_llm(
...
```python
async def call_tool_from_llm(self, tool_name: str, tool_input: str):
    try:
        # 尝试解析JSON
