...
本节介绍 AI Agent 系统在生产环境中的性能和成本优化策略，包括 Token 效率优化（Schema 缓存、提示词压缩）、延迟优化（流式响应、并发执行、多级缓存）、成本控制（多模型路由、模型选择、预算监控）以及性能基准测试框架。通过这些实践，可以实现显著的成本降低和性能提升。

### 10.3.1 概述

在生产环境中，AI Agent 系统的性能（延迟、吞吐量）和成本（API 调用费用、计算资源）是关键指标。本节介绍 Token 效率优化、延迟优化和成本控制的工程实践。

### 10.3.2 Token 效率优化

**Schema 缓存：** Schema 是工具参数的 JSON Schema 定义。在每次工具调用前，模型需要理解 Schema 来生成正确的参数。

**问题：** 相同的 Schema 在多个请求中重复发送，造成 Token 浪费。
...
| 缓存层次 | 作用域 | 命中条件 | 典型收益 |
|---------|--------|---------|---------|
| Schema 缓存 | 应用层 | 工具定义哈希匹配 | 减少重复 Token 发送 |
...
class MultiModelRouter:
    ...
    def __init__(self):
        self.models = {
            "simple": {"model": "haiku", "cost_per_mtok": 1.0},      # Haiku 4.5: $1/$5
            "moderate": {"model": "sonnet", "cost_per_mtok": 3.0},   # Sonnet 4.6: $3/$15
            "complex": {"model": "opus", "cost_per_mtok": 5.0},      # Opus 4.7: $5/$25
        }
    ...
    def select_model(
        ...
        for model, capabilities in self.MODEL_CAPABILITIES.items():
            ...
            if capabilities >= task_complexity:
                cost = self.estimate_cost(model)  # 按默认的 1000 入/500 出估算
...
**提示词缓存** 的成本收益来自缓存命中读取：Anthropic 对 cache read tokens 按基础输入 token 价格的 10% 计费（Fable 5.1 与 Mythos 5.1 为 2.5%）；首次写入缓存不是免费，5 分钟 TTL 写入按 1.25x 基础输入价、1 小时 TTL 写入按 2x 计费。长系统提示（Agent 系统常见）是否能节省 20-30% 账单，取决于静态前缀大小、命中率和写入刷新频率。缓存预热策略：预先加载常见系统提示变体。
...
模型路由通常可贡献 **40-60%** 的成本降低；叠加提示词前缀缓存、上下文裁剪和预算管控后，组合优化可达到 **60-80%**。上下文管理占总支出 60-70%，关键在于仅包含相关上下文，避免冗余数据。
...
多模型路由把请求引向更便宜的模型以压低单位 token 成本，但更低的单位成本常常是一种错觉。较弱的模型单步成功率也更低，一旦把失败、重试乃至人工兜底的代价折算进来，把任务交给弱模型反而可能更贵。真正应当优化的目标不是每 token 成本，而是**每成功任务成本**：

C_success ≈ C_token × E[尝试次数] / 成功率

公式里的 E[尝试次数] 与成功率都随模型能力和 Harness 的验证-重试闭环而变，本书已有的两处机制正好落在它上面：

- **压低分母侧的失败**：第 1.4.5 节的定量论证表明，结果验证加最多 3 次重试可把单步有效成功率从 p 提升到 p' = 1 − (1−p)^4，用更少的尝试摊薄每成功任务成本——省钱的杠杆常在 Harness，而非更贵的模型。
- **用正确的指标度量**：成本监控应以第 13.4 节 ProductionMetrics 的 cost_per_successful_task 字段为北极星，而非孤立地看 total_cost_usd 或平均 token 消耗。

这一转向也与政策导向一致：《北京市加快智能体引领发展若干措施》（京发改〔2026〕1185 号）第六条在发展 Token（词元）经济时，明确“鼓励创新主体从 Token 消耗量计费转向价值计费”（见附录参考文献）。
...
| 维度 | 优化方向 | 预期收益 |
|-----|--------|--------|
| Token 效率 | Schema 缓存、提示词压缩 | Schema token 30-50% 削减；总体按 30-40% 保守估算 |
| 延迟 | 流式处理、并发执行、缓存 | 端到端 Agent 请求 P99 参见第 11 章目标（<10 秒）；非 LLM 内部路径可按 <500ms 设计 |
| 成本 | 模型路由、缓存命中率、预算管控 | 路由单项 40-60%；组合优化 60-80% |
