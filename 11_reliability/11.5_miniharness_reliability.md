...
追踪用于“跟踪请求经过的所有函数调用”。MiniHarness 示例使用 OpenTelemetry/W3C 形态的 trace/span ID 和 JSON 追踪记录；若要接入完整 OpenTelemetry 生态，还需要补充 SpanContext、上下文传播和 OTLP exporter：
...
class Span:
    ...
    def __init__(self, name: str, trace_id: str, parent_id: str = None):
        self.span_id = secrets.token_hex(8)  # 8字节 SpanId，16位十六进制
        self.name = name
...
完整实现参见 `lab/mini_harness/reliability/resilience.py`（重试/熔断器）与 `lab/mini_harness/reliability/monitoring.py`（指标采集）。
