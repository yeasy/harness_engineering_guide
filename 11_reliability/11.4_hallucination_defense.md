...
LLM 幻觉（模型生成虚假、不准确或自相矛盾的内容）是 AI Agent 系统的主要风险。第 7 章介绍输出治理内的检测器，本节关注生产可靠性层的工程实践：运行策略、遥测、降级、人工审核、工具调用前验证、输出交叉检验和置信度评估。
...
Anthropic 官方 API 采用 `tool_use` 和 `tool_result` 的配对范式：`tool_result.tool_use_id` 必须匹配对应的 `tool_use.id`，并且结果块要按规定位置返回。这能保证消息结构可核对，但不证明工具输出内容真实、完整或可信。工具结果经常来自网页、邮件、用户上传和第三方 API，Harness 必须把这些内容视为不可信输入并自行做来源校验、内容验证和注入防护。

```python
# Anthropic 范式示例
# 1. Agent 生成 tool_use: {"id": "...", "tool": "search", "input": {...}}
# 2. Framework 执行工具，返回 tool_result: {"id": "...", "content": "..."}
# 3. Framework 校验 tool_result.tool_use_id 与 tool_use.id 一致
# 4. Harness 继续验证内容来源、格式、摘要和安全边界
```
