...
class ConsolidationEngine:
    ...
    def __init__(self, memory_store: MemoryStore):
        ...
        self.state = ConsolidationState(last_consolidation=datetime.now() - timedelta(days=1))

    def should_consolidate(self) -> bool:
        """检查是否应该触发整合"""
        # 三门条件
        time_gate = self.state.last_consolidation is None or (
            datetime.now() - self.state.last_consolidation > timedelta(hours=24)
        )
    ...
    async def orient_phase(self, recent_messages: List[str]) -> Dict[str, Any]:
        """阶段 1：确定方向"""
        # 简化实现：提取摘要
    ...
    async def consolidate_phase(self, gathered: Dict[str, List[str]]) -> bool:
        """阶段 3：融合到长期记忆"""
        try:
            # 更新用户档案
    ...
    async def prune_phase(self) -> int:
        ...
        for mem_type in ["episodic", "feedback"]:
            ...
            for mid in memory_ids:
                ...
                if entry and entry.confidence < 0.3 and entry.last_modified < cutoff:
