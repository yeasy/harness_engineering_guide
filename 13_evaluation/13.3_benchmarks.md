...
**SWE-bench Verified v2.0.0**（2026 年 2 月）：SWE-bench 进行了重大升级，更新了测试脚手架、执行环境和令牌限制。在新版基准上，GPT-5.2 以 80% 的解决率领跑排行榜，显著拉开了与其他系统的差距。同时，OpenAI Codex + GPT-5.2 在 Terminal-Bench 2.0 上达到 63% 的解决率。这一结果再次印证了 Harness 工程的重要性——同一底座模型在不同 Harness 中的表现差异巨大。
