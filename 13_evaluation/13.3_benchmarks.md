...
### 13.3.4 NIST AI Agent标准化倡议

2026年2月，NIST发起AI Agent标准化倡议，提出以下建议：
...
**SWE-bench Verified v2.0.0**（2026 年 2 月）：SWE-bench 进行了重大升级，更新了测试脚手架、执行环境和令牌限制。在新版基准上，OpenAI Codex + GPT-5.2 以 63% 的解决率领跑排行榜，显著拉开了与其他系统的差距。这一结果再次印证了 Harness 工程的重要性——同一底座模型在不同 Harness 中的表现差异巨大。
